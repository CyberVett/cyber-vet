...
const Login: React.FunctionComponent = () => {
  ...
  const submitLoginForm = (e: any) => {
    ...
    requestClient.post('users/login', {
      ...
      .then(response => {
        setLoading(false);
        if (response.status === 200  && response.data.data.role !== userRoles.SUPER_ADMINISTRATOR) {
        ...
        } else if ((response.status === 200  && response.data.data.role === userRoles.SUPER_ADMINISTRATOR)) {
          setLoading(false);
      ...
      .catch(error => {
        ...
        setError(['Login Failed!', error.data.message]);
      })
  };
